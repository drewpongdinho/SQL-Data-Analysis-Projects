...
SELECT 
    *
FROM
    cd.bookings; --Selecting relevant tables
...
FROM
    cd.facilities
WHERE
    membercost > 0; -- Displaying all facilities where there is a cost to members
...
WHERE
    membercost > 0
        AND membercost < (0.02 * monthlymaintenance); -- Where the member cost is greater than 0 and where the cost is less than 1/50th the monthly maintenance.
...
FROM
    cd.facilities
WHERE
    name LIKE '%Tennis%'; -- I use LIKE %Tennis% to highlight all sports that contain 'Tennis' in their name.
...
FROM
    cd.facilities
WHERE
    facid IN (1 , 5);-- Use of IN to highlight 1 and 5
-- OR
SELECT 
    *
FROM
    cd.facilities
WHERE
    facid = 1 OR facid = 5;  -- Use of OR to highlight 1 and 5
...
FROM
    cd.members
WHERE
    joindate BETWEEN '2012-09-01' AND NOW(); -- Select joindates between 2012-09-01 and now.
...
FROM
    cd.facilities
WHERE
    guestcost > 10.00; -- Display facilities where guestcost is >10 for a facility.
...
SELECT 
    facid, SUM(slots) AS total_slots
FROM
    cd.bookings
GROUP BY facid
...
SELECT 
    bookings.starttime, facilities.name
FROM
    cd.bookings
        INNER JOIN
    cd.facilities ON cd.bookings.facid = cd.facilities.facid
WHERE
    facilities.name IN ('Tennis Court 1' , 'Tennis Court 2')
...
WHERE
    members.firstname = 'David'
        AND members.surname = 'Farrell';  -- Where name = David Farrell
